...
from NODE.prompt import router_prompt,summarizer_prompt
...
from AUTO.node import make_auto_node
...
def final_answer_llm(state: KuramaState):
    ...
    for task in tasks:
        ...
        # ✅ Mark success vs failure clearly
        status = "❌ FAILED" if "error" in str(result).lower() or "failed" in str(result).lower() else "✅ DONE"
        formated_result += f"{status} {task.content}\nResult: {result}\n\n"
