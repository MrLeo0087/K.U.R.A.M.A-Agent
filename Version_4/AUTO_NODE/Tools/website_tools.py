...
def website_open(destination: str):
    """
    Intelligently opens a website. only if user ask to open website 
...
def search_youtube(query: str):
    """Searches YouTube for a specific video or topic. only use tool if user ask to search on youtube"""
...
def search_wikipedia(topic: str):
    """Opens the Wikipedia page for a specific topic.only use tool if user ask to search on wikipeida"""
...
def research_topic(topic: str):
    ...
    Extremely useful when you want to study something new. if user say deep research then use this tool
    """
    search_google(topic)
